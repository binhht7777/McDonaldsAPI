    ...
    if (userPhone != null) {
      try {
        ...
        const queryResult = await pool
          ...
          .query(
            "Select u.UserId, u.UserPhone, u.Name From Users u Where u.UserPhone = @userPhone"
          );
        if (queryResult.recordset.length > 0) {
          res.end(
            JSON.stringify({ success: true, result: queryResult.recordset })
  ...
  if (userId != null) {
    try {
      ...
      const queryResult = await pool
        ...
        .query(
          " Insert Into Users(UserId, UserPhone, Name) " +
            " Values(@userId, @userPhone, @name)"
        );
...
router.get("/background", async (req, res, next) => {
  ...
  if (req.query.key != API_KEY) {
    ...
    try {
      ...
      const queryResult = await pool
        .request()
        .query("Select bg.BackgroundId, bg.BackgroundImg From background bg");
      if (queryResult.recordset.length > 0) {
        res.end(
          JSON.stringify({ success: true, result: queryResult.recordset })
        );
  ...
  if (req.query.key != API_KEY) {
    ...
    try {
      ...
      const queryResult = await pool
        .request()
        .query(
          "Select c.CategoryId, c.CategoryName, c.Description, c.CategoryImg From [dbo].[Category] c"
        );
      if (queryResult.recordset.length > 0) {
        res.end(
          JSON.stringify({ success: true, result: queryResult.recordset })
...
router.get("/foodByCategory", async (req, res, next) => {
  console.log(req.query);
  if (req.query.key != API_KEY) {
    res.end(JSON.stringify({ success: false, message: "Wrong API Key" }));
  } else {
    ...
    if (categoryId != null) {
      try {
        ...
        const queryResult = await pool
          ...
          .query(
            "Select p.CategoryId, p.FoodId, p.FoodName, p.Price, p.FoodImg From Food p Where p.CategoryId = @categoryId"
          );
        if (queryResult.recordset.length > 0) {
          res.end(
            JSON.stringify({ success: true, result: queryResult.recordset })
...
//         .request()
//         .query(
//           "Select Id, Name, Address, Phone, Lat, Lng, UserOwner, 'http://192.168.1.4:3000/' + Image as Image, PaymentUrl From Restaurant"
//         );
//       if (queryResult.recordset.length > 0) {
//         res.end(
//           JSON.stringify({ success: true, result: queryResult.recordset })
...
//           .input("restaurantId", sql.Int, restaurantId)
//           .query(
//             "Select t.Id, t.Name, t.Description, 'http://192.168.1.4:3000/' + t.Image as Image From Menu t Where t.Id In " +
...
//           .query(
//             "Select Id, Name, Description, 'http://192.168.1.4:3000/' + Image as Image, Price, IsSize, IsAddon, Discount From Food Where Id In " +
...
//           .query(
//             "Select Id, Name, Description, 'http://192.168.1.4:3000/' + Image as Image, Price, IsSize, IsAddon, Discount From Food Where Id = @foodId "
//           );
//         if (queryResult.recordset.length > 0) {
//           res.end(
//             JSON.stringify({ success: true, result: queryResult.recordset })
...
//           .query(
//             "Select Id, Name, Description, 'http://192.168.1.4:3000/' + Image as Image, Price, IsSize, IsAddon, Discount From Food Where Name Like @searchQuery "
//           );
//         if (queryResult.recordset.length > 0) {
//           res.end(
//             JSON.stringify({ success: true, result: queryResult.recordset })
...
//           .input("Quantity", sql.Int, order_detail[i]["foodQuantity"])
...
//           .input("Fbid", sql.NVarChar, fbid)
//           .query(
//             "Select Fbid, FoodId, RestaurantId, RestaurantName, FoodName, 'http://192.168.1.4:3000/' + FoodImage as FoodImage, Price " +
